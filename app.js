document.addEventListener('DOMContentLoaded', () => {
    ...
    const clearDailyBudgetBtn = document.getElementById('clear-daily-budget');
    const clearMonthlyBudgetBtn = document.getElementById('clear-monthly-budget');
    ...
    clearDailyBudgetBtn.addEventListener('click', function() {
        dailyBudget = 0;
        monthlyBudget = 0;
        totalExpenses = 0;
        expenses = [];
        ...
        remainingBalanceElement.textContent = dailyBudget.toFixed(2);
        remainingMonthlyBalanceElement.textContent = monthlyBudget.toFixed(2);
        expensesList.innerHTML = '';
        savingsGoalsList.innerHTML = '';
        calendarEl.fullCalendar('removeEvents');
    });

    clearMonthlyBudgetBtn.addEventListener('click', function() {
        monthlyBudget = 0;
        localStorage.removeItem('monthlyBudget');
        monthlyBudgetDisplay.textContent = monthlyBudget.toFixed(2);
        updateRemainingMonthlyBalance();
    ...
    function addExpenseToTable(expense, index) {
        ...
        expenseRow.innerHTML = `
            ...
            <td class="border px-4 py-2">
                <button class="delete-expense bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600" data-index="${index}">Delete</button>
            </td>
        `;
    ...
    function addSavingsGoalToTable(goal, index) {
        ...
        goalRow.innerHTML = `
            ...
            <td class="border px-4 py-2">
                <button class="delete-savings-goal bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600" data-index="${index}">Delete</button>
            </td>
        `;
        savingsGoalsList.appendChild(goalRow);
    }

    savingsGoalsList.addEventListener('click', function(e) {
    ...
    function renderSavingsGoals() {
        savingsGoalsList.innerHTML = '';
        savingsGoals.forEach((goal, index) => addSavingsGoalToTable(goal, index));
    }
